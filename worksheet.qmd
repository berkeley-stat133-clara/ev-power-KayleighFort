...
```{r}
energy_clean <- tibble(source=clean_names, unit=energy_sources_unit)
energy_clean
```

e.  Discussion: Why might regex cleaning be important before performing joins or aggregations?\
    \
    prevents repetition of columns, helps clean up real world data, creates consistent keys for joining
...
#mistmatches for places like Puerto rico or DC
```
...
longer_socioeconomic <- states |>
  pivot_longer(cols=c("Income", "Population", "Illiteracy"), names_to="Variables", values_to="Values")

head(longer_socioeconomic)
```
...
```{r}
sales_longer <- pivot_longer(sales, cols=Jan_Sales:Mar_Sales, names_to="Month", values_to="Sales")
sales_longer
```
